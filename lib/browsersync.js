...
portfinder.getPort({
	...
	if (!err) {
		...
		bsync.watch('./app/assemble/fixtures/**/*.json', {
			awaitWriteFinish: {
			...
			switch (event) {
				...
				case 'add':
					...
					if (!initFlag) {
						// console.log('building assemble because of fixtures');
						exec('npm run assemble:execute && npm run assemble:build', (error, stdout, stderr) => {
							if (error) {
		...
		bsync.watch('./app/assemble/**/*.hbs', {
			...
			switch (event) {
				...
				case 'add':
					...
					if (!initFlag) {
						// console.log('building assemble from new file');
						exec('npm run assemble:execute && npm run assemble:build', (error, stdout, stderr) => {
							if (error) {
								console.error(`exec error: ${error}`);
								return;
